...
# ── 설정 ──────────────────────────────────────────────────
WORKSPACE_NAME = "260329"  # 작업 폴더명 (날짜, 논문명 등 자유롭게 지정)
...
OUTPUT_PATH = os.path.join(OUTPUT_DIR, f"{WORKSPACE_NAME}_presentation.pptx")
...
3. 이미지 매칭: 사용 가능한 파일명과 논문 캡션을 비교하여 적절한 슬라이드에 배치하세요.
   파일명이 "fig1.png" 형식이면 "Figure 1"에, 임의 파일명이면 캡션 내용으로 매칭하세요.
   각 파일은 한 슬라이드에서만 사용하세요.

4. asset_captions: 각 이미지 파일에 대한 설명 캡션을 작성하세요 (Figure/Table 번호 + 요약).

5. bullets는 한국어로 작성해주세요. 핵심만 간결하게.
